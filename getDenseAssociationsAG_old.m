...
agVertexList = [];
cmpHds = zeros(usvCount1, usvCount2);
% -- form the vertices
tic;
descr_threshold = 0.1;
for i = 1:usvCount1
   ...
   % v1Idx  = vData1(1,index1);
    ...
    for j = 1:usvCount2
       ...
       % v2Idx = vData2(1,index2);
        ...
        cmpHd = norm(hdv1 - hdv2);
        cmpHds(i,j) = cmpHd;
        % check if the vertex descriptors host enough information to
        % compare
        % *** TODO: Include this section and fix data vDescp ***
        
        %fprintf(1,'norm1: %d\tnorm2: %d\n',norm(hdv1), norm(hdv2));

        
        % compare the vertex descriptors
        if (cmpHd < descr_threshold)
            tmpVal = [index1, index2];
            agVertexTmpList = [agVertexTmpList;tmpVal];
            agIndexTmpList = [agIndexTmpList;j];
        end
    end
    if (size(agVertexTmpList,1) > neighbourLimit)
        agVertexTmpList = takeNeighbours(agVertexTmpList, agIndexTmpList, cmpHds(i,:), neighbourLimit);
    end
