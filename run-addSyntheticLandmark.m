    ...
    for j = 1:lCount
        ...
        if (lpDist < landmarkLookupRange * avgPseStep)
            ...
            if (landmarkIdx(j) == 0)
                ...
                if (isempty(find(landmarkIdx == tmpVarIDx)))
                    landmarkIdx(j) = tmpVarIDx + 1;
                end
                landmarkIdx(j) = vData(1,i) + 1;
...
%% export file to g2o
outFileName = 'outFileKitti.g2o';
exportLandmarkG2oFile(outFileName, vData, lData, eData, leData);

%% end/test code here
clear;
outFileName = 'outFileKitti.g2o';
[vertices,vCount,landmarks, lCount, edges,eCount, ledges, leCount, dim] =  readLandmarkG2oFile(outFileName , 'fc.txt', 1);
return;

%% project landmarks to the unoptimised dataset.
fileName = '../input/kitti00.g2o';
...
% -- convert data to matrix form
[vDataOrig, ~, ~, ~] = getMatrixForm(vertices, landmarks, edges, ledges);
...
shiftedLandmarkVertices = zeros(lCount, 1);

%% fix lData

% to consider the lData x,y information, we consider the leData
% information. For the ones, that have the v2 - v1 = 1, that means the
% landmark has started from that particular vertex, and hence we would use
% the landmark information from that particular index.
...
for i = 1:leCount
    ...
    v2 = leData(2,i);
    if ((v2 - v1) == 1)
        lIdx = find(lData(1,:) == v2);
        vIdx = find(vData(1,:) == v1);
        % -- for the x coord
        lDataOrig(2,lIdx) = ((vData(2,vIdx) - lData(2,lIdx)) + vDataOrig(2,vIdx));
        % -- for the y coord
        lDataOrig(3,lIdx) = ((vData(3,vIdx) - lData(3,lIdx)) + vDataOrig(3,vIdx));
    end
end

%% Check: Plot the new coordinates
figure('name','Unoptimised Graph');
color1 = jet(size(vData,2));
plotGraph(vDataOrig, lDataOrig, color1);



%% remove all loop closures
eDataOrig(:,lcEdgeIdx) = [];

%% export file as g2o
outFileName = 'outFileKitti.g2o';
exportLandmarkG2oFile(outFileName, vDataOrig, lDataOrig, eDataOrig, leDataOrig);
