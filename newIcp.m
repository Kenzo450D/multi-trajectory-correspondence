function [TR, TT, ER, t, match] = newIcp(q,p,initMatch, varargin)
...
inp.addRequired('initMatch', @(x)isreal(x));
...
inp.parse(q,p,initMatch,varargin{:});
arg = inp.Results;
clear('inp');
...
for k=1:arg.iter
    ...
    % Do matching
    if (k == 1)
        if (~isempty(initMatch))
            [match mindist] = icpTrajectory(q,pt, initMatch);
        else
            [match mindist] = icpTrajectory(q,pt);
        end
    else
